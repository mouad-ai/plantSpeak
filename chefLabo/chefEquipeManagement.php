    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <a href="./chefEquipeManagement.php">
                        <h2>Gestion <b>Des Botanistes</b></h2>
                    </a>
                    <div style="display: flex;">
                        <form action="" method="get" style="position: relative;">
                            <i class="fa-solid fa-magnifying-glass search-icon-admin"></i>
                            <input type="text" name="pid" class="search" placeholder="Par Nom De chef d'quipe">
                        </form>
                        <a class="btn add"><i class="fa-solid fa-square-plus"></i> <span>Ajouter Uu Chef
                                d'equipe</span></a>
                    </div>
                </div>
            </div>
            <table class="table table-striped table-hover">
                <thead>
                    <tr>
                        <th class="idC">ID de chef equipe </th>
                        <th>Prenom</th>
                        <th>Nom </th>
                        <th>Email</th>
                        <th>Mot de passe</th>
                ...
                <tbody>
                  ...
                  if (isset($_GET["pid"]) and $_GET["pid"]) {
                      $id = $_GET['pid'];
                     $sql = 'SELECT *  FROM chefequipe inner Join personne WHERE personne.PersonneID = chefequipe.ChefID and Nom = "'.$id.'" ';
                  } else {
                     $sql = 'SELECT *  FROM chefequipe inner Join personne WHERE personne.PersonneID = chefequipe.ChefID ';
                  }
                  ...
                  while ($botaniste = mysqli_fetch_assoc($res)) {
                     echo botaniste($botaniste['ChefID'], $botaniste['Nom'], $botaniste['Prenom'], $botaniste['Email'] , $botaniste['MotDePasse'],$botaniste['img']);
                  }
                  ?>
                </tbody>
            </table>
        ...
        btn.addEventListener('click', () => {
            ...
            if (botaniste) {
                manageBotaniste('supprimer', botaniste,
                    ...
                    `Ce chef d'quipe a été supprimé avec succès !`);
            }
        })
    })
        ...
        btn.addEventListener('click', () => {
            ...
            if (botaniste) {
                manageBotaniste('modifier', botaniste,
                    ...
                    `Ce chef d'equipe a été modifié avec succès !`);
            }
        })
    })
</script>
...
const addBotaniste = tr => {
    ...
    if (botaniste) {
        ...
        manageBotaniste('ajouter', botaniste,
            ...
            `Ce chef d'equipe a été ajouté avec succès !`);
    }
}
</script>